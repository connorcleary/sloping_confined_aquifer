...
def vertical_peclet2(K, n, alpha_v, D=8.64e-5): 
    return 2*(D+alpha_v*abs(vz(K, n)*n))/abs(vz(K, n)*n)
...
def horizontal_peclet(K, n,  z, L, alpha_l, D=8.64e-5):
    # return np.inf
    return 2*(D+alpha_l*abs(v_adv(K, n, z, L)))/abs(v_adv(K, n, z, L))
...
if __name__=="__main__":
    print(f"horizontal peclet transgression= {horizontal_peclet(0.0001, 0.2, 750, 150000, 0.1)}")
