...
def build_steady_model(pars):
    ...
    for cell in aquitard_cells: 
        ...
        k33[cell[0], cell[1], cell[2]] = pars.K_aquitard/pars.anis_aquifer
    ...
    sea_levels = [spl_time_to_sl(t0)] + [spl_time_to_sl(t0-(pars.dt/365000)*(i+1)) for i in range(nper_trans)] + [0]
    #pdb.set_trace()
    ghb_data = {}
    chd_data= {}
    cnc_data= {}
    ...
    for i in range(nper):
        ...
        if i >= 1:
            for cell in offshore_boundary_cells:  
            	if pars.Lx-cell[2]*pars.dx<=(pars.Lx-((pars.x_b*np.tan(pars.beta))-sea_levels[0])/np.tan(pars.beta))+(-spl_time_to_sl(t0) + spl_time_to_sl(t0-(pars.dt/(365000))*(i+1)))/np.tan(pars.beta): 
    ...
    flopy.mf6.ModflowGwfsto(gwf, ss=pars.ss_aquifer, sy=pars.sy_aquifer, iconvert=1, transient=True)
...
def get_results(name, scatter=False):
    pars = load_parameters(name)
