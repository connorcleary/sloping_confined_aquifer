...
def plot_metrics(name, times, axs2, axs3, axs4, axs5, axs3twin, axs4twin, axs5twin, y_labels="none", legend=False):
    ...
    # plot fresh
    axs2.plot(times[0::2], df.fresh_list)
    axs3twin.plot(times[0::2], df.toe_list)
    axs3twin.plot(times[0::2], df.tip_list)
    axs4.plot(times[0::2], df.volume_list)
    axs4twin.plot(times[0::2], df.centroid_list, linestyle="--")
    axs5.plot(times[0::2], df.fresh_flux_list)
    axs5.plot(times[0::2], df.sal_flux_list)
    axs5.set_xlabel("time from present (days)")
    axs5twin.plot(times[0::2], df.fresh_centroid_list, linestyle="--")
    axs5twin.plot(times[0::2], df.sal_centroid_list, linestyle="--")
